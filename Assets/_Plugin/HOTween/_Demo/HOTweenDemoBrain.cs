...
public class HOTweenDemoBrain : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        HOTween.To(CubeTrans2, 3, new TweenParms()
			.Prop("position", CubeTrans2.position + Vector3.up * 6, true) // Position tween (set as relative)
			.Prop("rotation", CubeTrans2.eulerAngles + Vector3.up * 1024, true) // Relative rotation tween (this way rotations higher than 360 can be applied)
        ...
        HOTween.To(this, 3, new TweenParms()
			.Prop("SampleString", "Hello I'm a sample tweened string")
        ...
        sequence.Append(HOTween.To(CubeTrans3, 1, new TweenParms()
			.Prop("rotation", Vector3.right * 360f, true)));
        sequence.Append(HOTween.To(CubeTrans3, 1, new TweenParms()
			.Prop("position", Vector3.up * 6f, true)));
        sequence.Append(HOTween.To(CubeTrans3, 1, new TweenParms()
			.Prop("rotation", Vector3.up * 360f, true)));
		...
		sequence.Insert(0, HOTween.To(CubeTrans3.GetComponent<Renderer>().material, sequence.duration, new TweenParms().Prop("color", colorTo)));
